...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    ...
    while(!qrCodesDetected) {
      cv::Mat curFrame;
      stream.read(curFrame);

      //read in original image
      int totalImgArea = curFrame.rows*curFrame.cols;
      int erodeMatCoeff = curFrame.rows/121;
      //initialize helper cv::Mat 's
      cv::Mat gray, thresh, threshErode, otsuTemp;
      //convert to grayscale
      cv::cvtColor(curFrame, gray, CV_RGBA2GRAY);

      //find the best thresholding value using otsu's algorithm
      //we want to only consider the area that contains the qr code paper
      cv::Rect myROI(0.2*curFrame.cols, 0.2*curFrame.rows, 0.5*curFrame.cols, 0.5*curFrame.rows);
      ...
      for (int i = 0; i < contours.size(); ++i) {
        ...
        if (poly.rows == 4 && area < 0.53*totalImgArea && area > 0.002*totalImgArea) {
          ...
          drawContours( curFrame, contours, i, color, 2, 8, hierarchy, 0, cv::Point() );
          count ++;
        }
      }
      if (count > 16){
